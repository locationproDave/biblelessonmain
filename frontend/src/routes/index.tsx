...
function TestimonialsSection() {
  ...
  const testimonials = [
    {
      quote: 'Bible Lesson Planner has transformed my Sunday school preparation. What used to take hours now takes minutes, and the quality is incredible.',
      name: 'Sarah M.',
    ...
    {
      quote: 'The age-appropriate activities are spot on. My preschoolers are more engaged than ever, and the Bible stories really come alive.',
      name: 'Pastor James R.',
      role: 'Youth Pastor',
    ...
    {
      quote: 'As a volunteer teacher, I was always stressed about lesson planning. Bible Lesson Planner gives me confidence every Sunday morning.',
      name: 'Maria L.',
  ...
  return (
    <section ref={ref} className="py-10 sm:py-14 bg-gradient-to-b from-transparent via-[#F5F8FC]/15 to-transparent dark:via-[#1E293B]/15">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <span className="inline-block px-3 py-1 rounded-full bg-[#F5F8FC] dark:bg-[#1E3A5F]/40 text-[#1E3A5F] dark:text-[#D4A017] text-xs font-bold uppercase tracking-wider mb-3">{t('testimonials.badge')}</span>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-[#333333] dark:text-white">
            {t('testimonials.title')}
          </h2>
          <p className="mt-2 text-base text-[#6B7280] dark:text-[#94A3B8]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {testimonials.map((item, i) => (
            <div
              key={i}
              className={`relative p-5 rounded-2xl bg-white dark:bg-[#0F1F33]/60 border transition-all duration-500 ${
                activeIndex === i
                  ? 'border-[#D4A017]/40 dark:border-[#D4A017]/40 shadow-lg shadow-[#D4A017]/15 dark:shadow-blue-900/20 scale-[1.02]'
                  : 'border-[#E8DCC8] dark:border-[#334155] shadow-sm'
            ...
            >
              ...
              <p className="text-[#4B5563] dark:text-[#CBD5E1] leading-relaxed mb-4 italic text-sm">
                &ldquo;{item.quote}&rdquo;
              </p>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#1E3A5F] to-[#D4A017] flex items-center justify-center text-white font-bold text-sm">
                  {item.initials}
                </div>
                <div>
                  <p className="font-bold text-[#333333] dark:text-white text-sm">{item.name}</p>
                  <p className="text-xs text-[#6B7280] dark:text-[#94A3B8]">{item.role}</p>
                  <p className="text-xs text-[#9CA3AF] dark:text-[#64748B]">{item.church}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                activeIndex === i ? 'w-6 bg-[#D4A017]' : 'bg-[#CBD5E1] dark:bg-[#475569]'
              }`}
            />
          ))}
        </div>
